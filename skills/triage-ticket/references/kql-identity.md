...
On **TOS**, pull these through the MS Graph Enterprise MCP (directory-audit identity info), not
...
```
For non-TOS clients the same data is in `AuditLogs`:
```kusto
AuditLogs
| where TimeGenerated > ago(14d)
...
**Found (TOS-38623):** two privileged accounts (`jetringerx@` Global Admin, `clinnerx@` User/Groups/
...
| `has` misses substrings in a UPN | `Actor has "mischlich"` returns **zero rows** — `has` matches whole tokens, and `bmischlich@…` tokenises to `bmischlich`/`authoritybrandsllc`/`com` | Use `contains` for partial matches inside UPNs/emails; reserve `has` for whole-word matches (e.g. a display name in `TargetResources`) |
...
| `70045` | Refresh token invalid — CA sign-in-frequency | Token | Refresh-token replay rejected at issuance; **no session granted**, and the token is now dead ("will never be usable"). Seen on TOR/anonymized-IP token replay (AUT-5524) |
...
is a *near-miss* (contain), not access (AUT-5524).
...
> a privileged user re-enrolling MFA on a new phone from home is the common benign shape (TOS-38623),
